...
internal class ExternalAccountLoginCommandHandler : ICommandHandler<ExternalAccountLoginCommand, AuthenticationResult>
{
    ...
    public async Task<AuthenticationResult> Handle(ExternalAccountLoginCommand request, CancellationToken cancellationToken)
    {
        ...
        // This will return the user we have that is linked to this external account
        ...
        if (user is null)
        {
            ...
            if (!string.IsNullOrEmpty(emailAddress))
            {
                ...
                if (user == null)
                {
                    ...
                    if (!Email.IsValid(emailAddress, out Error? error))
                    {
                    ...
                    user = ApplicationUser.CreateUser(new UserId(Guid.NewGuid()), emailAddress, email, null, null, null).Value;
