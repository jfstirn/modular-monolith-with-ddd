...
namespace CompanyName.MyMeetings.Modules.UsersMI.Infrastructure.Services.IdentityTokenService;

internal class IdentityTokenClaimService : Application.Contracts.ITokenClaimsService
{
    private const int DefaultJwtTokenLifetimeInMinutes = 7;
    ...
    public Application.Contracts.Tokens GenerateTokens(ApplicationUser user)
    {
        ...
        var tokenDescriptor = new SecurityTokenDescriptor
        {
            ...
            Expires = DateTime.UtcNow.AddMinutes(_userAccessConfiguration.Security?.JwtTokenLifetimeInMinutes ?? DefaultJwtTokenLifetimeInMinutes),
        ...
        return new Application.Contracts.Tokens(accessToken, refreshToken);
    }

    public async Task<Result<Application.Contracts.Tokens, Error>> GenerateNewTokensAsync(string accessToken, string refreshToken, CancellationToken cancellationToken)
    {
    ...
    public List<Claim> GetUserClaims(ApplicationUser user)
    {
        ...
        {
            new(CustomClaimTypes.Sub, user.Id.ToString()),
            new(CustomClaimTypes.LoginName, user.UserName ?? string.Empty),
            new(CustomClaimTypes.Email, user.Email ?? string.Empty)
        };

        if (!string.IsNullOrEmpty(user.Name))
        {
            claims.Add(new(CustomClaimTypes.Name, user.Name));
        }

        if (!string.IsNullOrEmpty(user.FirstName))
        {
            claims.Add(new(CustomClaimTypes.FirstName, user.FirstName));
        }

        if (!string.IsNullOrEmpty(user.LastName))
        {
            claims.Add(new(CustomClaimTypes.LastName, user.LastName));
        }
